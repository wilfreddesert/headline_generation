...
python3 train.py -task abs -mode train -bert_data_path "$BERT_RIA_PATH" -visible_gpus 3 -dec_dropout 0.2 -model_path /data/alolbuhtijarov/model/presum -sep_optim true -lr_bert 0.002 -lr_dec 0.2 -save_checkpoint_steps 94 -batch_size 140 -train_steps 100 -report_every 10 -accum_count 5 -use_bert_emb true -use_interval true -warmup_steps_bert 20000 -warmup_steps_dec 10000 -max_pos 512 -log_file ../logs/abs_bert_ria;

#python3 train.py -task abs -mode validate -batch_size 10 -visible_gpus 3 -test_batch_size 500 -bert_data_path "$BERT_RIA_PATH" -log_file ../logs/val_abs_bert_ria -model_path /data/alolbuhtijarov/model/presum -sep_optim true -use_interval true -max_pos 512 -max_length 200 -alpha 0.95 -min_length 50 -result_path ../results/
